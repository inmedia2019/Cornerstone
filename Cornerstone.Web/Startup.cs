...
using Cornerstone.Code.Model;
using System.Text.Unicode;
using Newtonsoft.Json.Serialization;
using Cornerstone.Service.AutoJob;
using UEditor.Core;
...
using Quartz.Spi;
using System;
using Cornerstone.Domain.Entity.Generate;
    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.Configure<KestrelServerOptions>(options =>
            {
                ...
                options.Limits.MaxRequestBodySize = long.Parse(Configuration.GetSection("SystemConfig:Kestrel").Value);
            });
            //上传文件大小限制IIS设置
            services.Configure<IISServerOptions>(options =>
            {
                options.MaxRequestBodySize = long.Parse(Configuration.GetSection("SystemConfig:Kestrel").Value);
            });
        ...
        {
            var assemblys = Assembly.Load("Cornerstone.Service");//Service是继承接口的实现方法类库名称
...
            builder.RegisterAssemblyTypes(assemblys).Where(m => baseType.IsAssignableFrom(m) && m != baseType)
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IHostApplicationLifetime appLifetime)
        {
            ...
            app.UseEndpoints(endpoints =>
            {
                ...
                endpoints.MapControllerRoute("english", "{area:exists}/{controller=Homes}/{action=Index}/{id?}");
